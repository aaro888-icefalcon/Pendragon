...
| Thread pressure | every scene | Light nudge: check each open thread in `state/threads.json` against its pressure line; surface the nearest if the scene stalled. Full advance every ignored thread one step at Winter Phase — rivals gain Glory, grudges fester, opportunities expire (`gm/PACING.md` §4). |
...
| Faction turn (off-screen) | every 3 scenes | Advance one off-screen power: roll `generators/factions_faction_goal.json` (or run a Faction Turn per `generators/02-factions.md`) plus a Mythic Move-Toward/Away an open Thread. Record standings to `state/characters.json` / `state/threads.json`. |
